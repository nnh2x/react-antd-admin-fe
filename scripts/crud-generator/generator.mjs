...
// The action column deliberately has no `fixed: "right"` — antd Table pins a fixed column to the
// *container's* right edge, not the table's own right edge, so on a table whose columns don't
// fill the container (typical for a freshly generated module with just a few fields) it leaves a
// dead gap between the last regular column and the floating fixed one. Add `fixed: "right"` back
// by hand once the table has enough columns to actually need horizontal scrolling.
